<x-layouts.app>
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex gap-2">
            <a href="{{ route($routePrefix . 'chick-rearings.create') }}" wire:navigate class="inline-flex items-center px-4 py-2 bg-emerald-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-emerald-700 focus:bg-emerald-700 active:bg-emerald-900 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 transition ease-in-out duration-150">
    ...
    <div class="mb-6 bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
        <form method="GET" action="{{ route($routePrefix . 'chick-rearings.index') }}" class="flex flex-wrap gap-4 items-end">
            ...
            <div class="w-40">
                <label for="growth_stage" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Growth Stage</label>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                <thead class="bg-gray-50 dark:bg-slate-900">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Vaccination Status') }}</th>
                ...
                <tbody class="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700">
                    @forelse ($chickRearings as $chickRearing)
                        <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                @php
                                    $vacColor = match($chickRearing->vaccination_status) {
                                        'Completed' => 'green',
                                        'Pending' => 'yellow',
                                        'N/A' => 'gray',
                                        default => 'gray',
                                ...
                                @endphp
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-{{ $vacColor }}-100 text-{{ $vacColor }}-800 dark:bg-{{ $vacColor }}-900 dark:text-{{ $vacColor }}-300">
                                    {{ $chickRearing->vaccination_status ?: 'N/A' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end gap-2">
                                    <a href="{{ route($routePrefix . 'chick-rearings.show', $chickRearing) }}" wire:navigate class="text-emerald-600 dark:text-emerald-400 hover:text-emerald-900 dark:hover:text-emerald-300">{{ __('View') }}</a>
                                    <a href="{{ route($routePrefix . 'chick-rearings.edit', $chickRearing) }}" wire:navigate class="text-yellow-600 dark:text-yellow-400 hover:text-yellow-900 dark:hover:text-yellow-300">{{ __('Edit') }}</a>
                                    <form action="{{ route($routePrefix . 'chick-rearings.destroy', $chickRearing) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this record?');">
                                        @csrf
                                        @method('DELETE')

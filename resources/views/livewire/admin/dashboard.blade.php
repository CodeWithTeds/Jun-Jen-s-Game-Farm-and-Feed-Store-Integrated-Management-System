    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
            <div class="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
                <h3 class="text-lg font-medium text-slate-900 dark:text-slate-100">Recent Orders</h3>
                <a href="{{ route('admin.sales-transactions.index') }}" class="text-sm text-emerald-600 hover:text-emerald-500 dark:text-emerald-400">View All</a>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left">
                    <thead class="text-xs text-slate-500 uppercase bg-slate-50 dark:bg-slate-800/50 dark:text-slate-400">
                        <tr>
                    ...
                    <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
                                ...
                                <td class="px-6 py-4">
                                    <span class="px-2 py-1 text-xs rounded-full 
                                        {{ $order->status === 'completed' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' : 
                                           ($order->status === 'pending' ? 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400' : 'bg-slate-100 text-slate-800 dark:bg-slate-800 dark:text-slate-400') }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-4 text-center text-slate-500">No recent orders.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
             <div class="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
                <h3 class="text-lg font-medium text-slate-900 dark:text-slate-100 text-red-600 dark:text-red-400">Low Stock Alerts</h3>
                <a href="{{ route('admin.products.index') }}" class="text-sm text-emerald-600 hover:text-emerald-500 dark:text-emerald-400">Manage Inventory</a>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left">
                    <thead class="text-xs text-slate-500 uppercase bg-slate-50 dark:bg-slate-800/50 dark:text-slate-400">
                    ...
                    <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-6 py-4 text-center text-slate-500">No low stock items.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>

    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-zinc-900 overflow-hidden shadow-sm sm:rounded-lg border border-zinc-200 dark:border-zinc-700 p-6">
                ...
                <form action="{{ route('staff.breedings.store') }}" method="POST" class="space-y-6">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="date" name="breeding_date" id="breeding_date" value="{{ old('breeding_date') }}" class="mt-1 block w-full rounded-md border-gray-300 dark:border-zinc-700 dark:bg-zinc-900 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                        </div>

                        <!-- Expected Hatch Date -->
                        <div>
                            <label for="expected_hatch_date" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Expected Hatch Date</label>
                            <input type="date" name="expected_hatch_date" id="expected_hatch_date" value="{{ old('expected_hatch_date') }}" class="mt-1 block w-full rounded-md border-gray-300 dark:border-zinc-700 dark:bg-zinc-900 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="pen_number" id="pen_number" value="{{ old('pen_number') }}" class="mt-1 block w-full rounded-md border-gray-300 dark:border-zinc-700 dark:bg-zinc-900 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                        </div>

                        <!-- Clutch Number -->
                        <div>
                            <label for="clutch_number" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Clutch / Batch Number</label>
                            <input type="text" name="clutch_number" id="clutch_number" value="{{ old('clutch_number') }}" class="mt-1 block w-full rounded-md border-gray-300 dark:border-zinc-700 dark:bg-zinc-900 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        </div>
                    ...
                    <div>
                        ...
                        <textarea name="notes" id="notes" rows="4" class="mt-1 block w-full rounded-md border-gray-300 dark:border-zinc-700 dark:bg-zinc-900 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">{{ old('notes') }}</textarea>
                    </div>

                    <div class="flex justify-end gap-4">
                        <a href="{{ route('staff.breedings.index') }}" wire:navigate class="inline-flex items-center px-4 py-2 bg-white dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-md font-semibold text-xs text-gray-700 dark:text-gray-300 uppercase tracking-widest shadow-sm hover:bg-gray-50 dark:hover:bg-zinc-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-25 transition ease-in-out duration-150">
                            Cancel
                        </a>

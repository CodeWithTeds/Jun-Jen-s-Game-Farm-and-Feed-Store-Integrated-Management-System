<x-layouts.app>
    <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex gap-2">
            <a href="{{ route('admin.suppliers.create') }}" wire:navigate class="inline-flex items-center px-4 py-2 bg-emerald-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-emerald-700 focus:bg-emerald-700 active:bg-emerald-900 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 transition ease-in-out duration-150">
                {{ __('Add New Supplier') }}
            </a>
        </div>
    </div>
    ...
    <div class="mb-6 bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
        <form method="GET" action="{{ route('admin.suppliers.index') }}" class="flex flex-wrap gap-4 items-end">
            ...
            <div class="w-40">
                <label for="supplier_type" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Supplier Type</label>
                <select id="supplier_type" name="supplier_type" class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-300 dark:focus:border-emerald-500 transition-shadow">
                    ...
                    @forelse ($suppliers as $supplier)
                        <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <a href="{{ route('admin.suppliers.show', $supplier->id) }}" wire:navigate class="text-emerald-600 hover:text-emerald-900 dark:text-emerald-400 dark:hover:text-emerald-300 mr-3">{{ __('View') }}</a>
